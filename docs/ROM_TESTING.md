...
`--dump-analysis` reports the detected `TMR SEGA` header offset, region/size byte, stored checksum, and checksum over the declared ROM size excluding the 16-byte header block. It also reports the conservative image heuristic used when a standard header is absent: `.gg` selects Game Gear, `.sg`/`.sc` selects SG-3000, `.sms` selects Master System export, and small headerless boot images are marked `image_bios_like` without forcing a model. Use this before mapper/BIOS debugging to catch wrong-platform images or surprising size metadata.
...
- Four private images now reach gameplay through deterministic input. One completed a five-minute NTSC regression with continuous audio, save-state round-trip, and accurate/enhanced comparison; three others completed 1,200-frame gameplay probes with audio.
