...
`--run-host` uses the headless host runtime path. It advances full frames, samples audio at 44.1 kHz, applies joypad state through the runtime API, and exposes the latest framebuffer for a future window backend. `HostRuntimeConfig` now drives the VDP scanline/frame timing used by the host loop; pass `--video-standard ntsc|pal` or set `video_standard = "pal"` in a local profile to switch frame timing without storing ROM paths.
...
SG-3000 consoles select an initial TMS9918-style Graphics I renderer path. It currently covers background tiles through the TMS name, pattern, color tables, and palette; sprites and the other legacy TMS modes are still roadmap items.
